...
When to use tools:
- creative_write: use this when you need to generate new creative content like plot_overview or descriptions
- set_properties: ONLY use after:
  * Getting explicit user approval
  * Having complete information for the properties
  * Verifying they're valid properties (title, genre, plot_overview, etc.)
...
- Don't call set_properties without explicit user approval and complete information
- Only call set_properties if the new values are different from the old values
- Don't tell the user you set properties unless you used the set_properties tool
- If set_properties is dependent on a previous creative_write, then you must call creative_write first
...
- Any time you have information that can be used to set properties, you should set those properties, whether from a tool call or user input
- Any time you see any hard requirements from the user, append it to the requirements field. Be sure to include all current requirements as well. An example of a hard requirement is a specific character is requested or restrictions like no child harm.
...
- Medium is something like a book, movie, tv show, etc and we need the user input for this.
- Keep the story consistent. If something isn't quite adding up, ask the user for clarification.
- plot_overview must be generated by the user or the creative_write tool.
- If the plot_overview is very vague, work with the user to expand it.
"""
    ...
    def __init__(self, 
        ...
        self.register_tool('set_properties')  # Enable built-in set_properties
    ...
    def _format_story_state(self) -> str:
        ...
        for field_name, field in self.story.model_fields.items():
            ...
            # Add the field description with type information
            field_type = str(field.annotation).replace('typing.', '')  # Remove typing. prefix
            field_descriptions.append(f"{field_name} ({field_type}): {field.description}")
